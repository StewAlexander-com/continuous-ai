...
def test_guard_has_temporal_integrity():
    ...
    assert "recite" in g
    print("[PASS] _GUARD_TEXT encodes TEMPORAL INTEGRITY (silent clock)")
...
def test_runtime_clock_line_delegates_to_voice():
    ...
    assert "do not" in line.lower() and "recite" in line.lower()
...
def test_voice_prompt_still_reinforces():
    ...
    print("[PASS] per-turn voice line leads with SYSTEM CLOCK (silent)")
