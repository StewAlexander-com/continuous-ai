...
In chat: `:search <what>` — Aida interprets any English ask, searches, then reviews hits with a path:line contract. Optional specificity: token, `"quoted"`, `name <pat>`, `<what> /path/file.txt`, `in <dir> depth 1|3|all`. `rg` is the engine, not the product.
