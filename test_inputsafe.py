...
def test_blank_line_submits_block():
    out = I.read_multiline(_input=_feeder(["line one", "line two", ""]), _isatty=True)
    check("multi-line block assembled to blank line", out == "line one\nline two")


def test_empty_first_line_is_empty_turn():
    out = I.read_multiline(_input=_feeder([""]), _isatty=True)
...
def test_single_line_command_returns_immediately():
    ...
    check("command returned without block mode", out == ":model llama3")
...
def test_eof_during_block_submits_what_there():
    # no trailing blank line -> EOFError -> submit gathered lines
    out = I.read_multiline(_input=_feeder(["a", "b"]), _isatty=True)
    check("EOF mid-block submits gathered text", out == "a\nb")


def test_eof_on_first_line_raises():
    raised = False
    try:
        I.read_multiline(_input=_feeder([]), _isatty=True)
    except EOFError:
        raised = True
...
def test_seedling_gate_blocks_commands_in_multiline(_unused=None):
    ...
    typed = I.read_multiline(_input=_feeder(["exit"]), _isatty=True)
    check("typed 'exit' alone returns 'exit' (single-line quit, expected)", typed == "exit")
    block = I.read_multiline(_input=_feeder(["please summarize:", "exit codes 0-255", ""]), _isatty=True)
    check("a real multi-line block stays multi-line text", "\n" in block and block.startswith("please summarize"))
...
if __name__ == "__main__":
    for fn in [
        ...
        test_blank_line_submits_block, test_empty_first_line_is_empty_turn,
        test_single_line_command_returns_immediately,
        test_eof_during_block_submits_what_there, test_eof_on_first_line_raises,
