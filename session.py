...
_GUARD_TEXT = (
    ...
    "ISO timestamp. It IS today where you and the user are — silent orientation, "
    "not a stamp to paste into every reply. Recite the date/time ONLY when the "
    "user asks, or the question truly has a time dimension; otherwise answer "
    "normally and do not mention the clock. Your training knowledge end-date is "
    "coverage of the world, NOT the calendar and NOT a reason to refuse a "
    "time-dimension question wholesale. Never claim a past month or year 'has "
    "not yet occurred.' When a question has a time dimension: orient from the "
    "system clock first, then answer from what you actually recall; mark "
    "uncertain dates or scores; do not invent names or benchmark numbers; do "
    "not replace an answer with a knowledge-cutoff monologue; do not stretch "
    "unrelated earned beliefs as analogies for refusing. When asked about the "
    "model id you are running, treat that identity as known. Gaps in coverage "
    "stay honest hedges on specific items — they do not rewind the clock and "
    "they do not forbid trying."
    + "\n\n"
